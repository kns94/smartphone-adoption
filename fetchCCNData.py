""" Extract TAC Ids from a CCN's Osmocon database """
...
class fetchCCNData:
    ...
    def __init__(self, connection_arguments, ccn_db, cdr_count_file, output_directory):
        """Initializing Databases. The CCN database which is in sqlite3 is refered to as ccn_db and 
        ...
        self.ccn_conn = sqlite3.connect(ccn_db, timeout = 100)
        self.ccn_cur = self.ccn_conn.cursor()
    ...
    def fetchTAC(self):
        ...
        for data in subscriber_id:
            self.ccn_cur.execute("SELECT IMEI FROM Equipment WHERE id = (SELECT equipment_id FROM EquipmentWatch WHERE subscriber_id = {s_id});".format(s_id = data[0]))
            parsed_imei = self.ccn_cur.fetchall()
...
def main():
        ...
        if args.ccn_db:
            ccn_db = "{}/{}".format(current_dir, args.ccn_db)
        else:
            ccn_db = "{}/{}".format(current_dir, 'ccn_db')
            if args.dump_file:
                ccn_db_file = "{}/{}".format(current_dir, args.dump_file)
                preProcess().importToSQLITE(ccn_db_file, ccn_db)
            else:
        ...
        crawler = fetchCCNData(bst_connection_arguments, ccn_db, cdr_count_file, output_directory)
        crawler.fetchTAC()
